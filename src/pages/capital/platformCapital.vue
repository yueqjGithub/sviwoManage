<template>
  <div>
    ...
    <div class="searchDiv">
      <el-form :size="'mini'" :label-position="'left'"  :inline="true" :rules="rule">
        ...
        <el-form-item
          label="类型"
        >
          <el-select v-model="search.flowType"  clearable placeholder="全部">
            <el-option
    ...
    <div class="cuntOrder">
      <div class="cuntDiv">
        <span class="cuntTit">收入金额：</span>
        <countTo :startVal='0' :endVal='sum.expenditure' :duration='1500' class="cuntCount"
                 :decimals="2"
        ></countTo>
      </div>
      <div class="cuntDiv">
        <span class="cuntTit">支出金额：</span>
        <countTo :startVal='0' :endVal='sum.income' :duration='1500' class="cuntCount"
                 :decimals="2"
        ></countTo>
      </div>
      <div class="cuntDiv">
        <span class="cuntTit">记录总数：</span>
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        ...
        selectArr: {
          ...
          businessArr: [{label: '优惠减免', value: '3011'}, {label: '司机奖励', value: '3013'}, {label: '订单补贴', value: '3014'},
            {label: '订单提成', value: '3015'}]
        },
        rule: {},
        sum: {
    ...
    methods: {
      ...
      getSum (opt) { // 获取统计
        axios_post.post(urls.accountFlowTitle,opt).then(res => {
          let vm = this
          let code = res.data.code
          if (code === '10000') {
          ...
          vm.$message.error('获取平台流水统计失败，请联系管理员')
        })
      },
      searchFlow () { // 搜索
        let param = {
      ...
      exportFlow () { // 导出 exportOrder
        ...
        this.$axios.post(urls.exportPlatformAccFlow,fm, {
          headers:{
