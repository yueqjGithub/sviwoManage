<template>
    <div>
        <div class="col-md-12 search_more_box">
            <div class = "input-group col-md-5 col-lg-5 col-sm-5 col-xs-5">
                <label>司机昵称</label>
                <input type="text" placeholder="司机昵称" class="search_input" v-model="s_driverName">
            </div>
            <div class = "input-group col-md-5 col-lg-5 col-sm-5 col-xs-5">
                <label>证件类型</label>
                <select name="" class="search_input" v-model="s_certificateType" autocomplete="off">
                    <option value='' disabled selected style='display:none;'>请选择</option>
                    <option value="1" selected="selected">身份证</option>
                </select>
            </div>
            <div class = "input-group col-md-5 col-lg-5 col-sm-5 col-xs-5">
                <label>证件号</label>
                <input type="text" placeholder="证件号" class="search_input" v-model="s_certificateNo">
            </div>
            <div class = "input-group col-md-5 col-lg-5 col-sm-5 col-xs-5">
                <label>认证状态</label>
                <!--<input type="text" placeholder="认证状态" class="search_input" v-model="s_driverStatus">-->
                <select name="" class="search_input" v-model="s_driverStatus" placeholder="请选择" v-if="!isAudit">
                    <option value='' disabled selected style='display:none;'>请选择</option>
                    <option value="3">已认证</option>
                    <option value="4">已封号</option>
                </select>
                <select name="" class="search_input" v-model="s_driverStatus" placeholder="请选择" v-if="isAudit">
                    <option value='' disabled selected style='display:none;'>请选择</option>
                    <!--<option value="0">未认证</option>-->
                    <option value="1">审核中</option>
                    <option value="2">认证失败</option>
                </select>
            </div>
            <div class = "input-group col-md-5 col-lg-5 col-sm-5 col-xs-5">
                <label>真实姓名</label>
                <input type="text" placeholder="真实姓名" class="search_input" v-model="s_realName">
            </div>
            <div class = "input-group col-md-5 col-lg-5 col-sm-5 col-xs-5">
                <label>注册IP地址</label>
                <input type="text" placeholder="注册IP地址" class="search_input" v-model="s_registerIp">
            </div>
            <div class = "input-group col-md-5 col-lg-5 col-sm-5 col-xs-5">
                <label>司机类型</label>
                <!--<input type="text" placeholder="司机类型" class="search_input" v-model="s_driverType">-->
                <select name="" class="search_input" v-model="s_driverType">
                    <option value='' disabled selected style='display:none;'>请选择</option>
                    <option value="1">签约司机</option>
                    <option value="2">加盟司机</option>
                </select>
            </div>
            <div class = "input-group col-md-5 col-lg-5 col-sm-5 col-xs-5">
                <label>创建时间</label>
                <input type="datetime-local" placeholder="创建时间" class="search_input" v-model="s_createTime">
            </div>
            <div class = "input-group col-md-5 col-lg-5 col-sm-5 col-xs-5">
                <label>地区名</label>
                <input type="text" placeholder="地区名" class="search_input" v-model="s_areaName">
            </div>
            <div class = "input-group col-md-5 col-lg-5 col-sm-5 col-xs-5">
                <label>行驶证号</label>
                <input type="text" placeholder="行驶证号" class="search_input" v-model="s_licenseNumber">
            </div>
            <div class = "input-group col-md-5 col-lg-5 col-sm-5 col-xs-5">
                <label>车牌号</label>
                <input type="text" placeholder="车牌号" class="search_input" v-model="s_licensePlates">
            </div>
            <div class = "input-group col-md-5 col-lg-5 col-sm-5 col-xs-5">
                <label>修改人</label>
                <input type="text" placeholder="修改人" class="search_input" v-model="s_updateUserName">
            </div>
            <div class = "input-group col-md-5 col-lg-5 col-sm-5 col-xs-5">
                <label>app用户名</label>
                <input type="text" placeholder="app用户名" class="search_input" v-model="s_userName">
            </div>
            <p class="col-md-12 col-lg-12 col-sm-12 col-xs-12"></p>
            <div class = "input-group col-md-5 col-lg-5 col-sm-5 col-xs-5">
                <label>修改开始时间</label>
                <input type="datetime-local" placeholder="修改开始时间" class="search_input" v-model="s_updateTimeStart">
            </div>
            <div class = "input-group col-md-5 col-lg-5 col-sm-5 col-xs-5">
                <label>修改结束时间</label>
                <input type="datetime-local" placeholder="修改结束时间" class="search_input" v-model="s_updateTimeEnd">
            </div>
            <!--<div class = "input-group col-md-5 col-lg-5 col-sm-5 col-xs-5">-->
                <!--<label>创建开始时间</label>-->
                <!--<input type="datetime-local" placeholder="创建开始时间" class="search_input" v-model="s_createTimeStart">-->
            <!--</div>-->
            <!--<div class = "input-group col-md-5 col-lg-5 col-sm-5 col-xs-5">-->
                <!--<label>创建结束时间</label>-->
                <!--<input type="datetime-local" placeholder="创建结束时间" class="search_input" v-model="s_createTimeEnd">-->
            <!--</div>-->
            <div class = "input-group col-md-5 col-lg-5 col-sm-5 col-xs-5" v-show="1===2">
                <label>创建结束时间</label>
                <input type="datetime-local" placeholder="创建结束时间" class="search_input">
            </div>
            <div class="btnGroup col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <input class="btn btn-default searchBtn" type="button" value="高级搜索" @click="search_More">
                <input class="btn btn-default searchBtn" type="button" value="重置" @click="clearOption">
                <input class="btn btn-default searchBtn" type="button" value="关闭" @click = 'offSelf'>
                <!--<input class="btn btn-default searchBtn" type="button" value="关闭" @click = 'test'>-->
            </div>
        </div>
    </div>
</template>

<script>
    import serachOption from '@/model/searchOption.js'
    export default {
        name: 'searchMore',
        props:['isAudit'],
        data: function () {
            return {
                s_driverName: '',
                s_certificateType: '',
                s_certificateNo: '',
                s_driverStatus: '',
                s_realName: '',
                //注册IP地址
                s_registerIp: '',
                // 司机类型
                s_driverType: '',
                // 创建时间
                s_createTime: '',
                // 地区名
                s_areaName: '',
                s_licenseNumber: '',
                s_licensePlates: '',
                // 修改人
                s_updateUserName: '',
                s_userName: '',
                s_updateTimeStart:'',
                s_updateTimeEnd:'',
//                s_createTimeStart: '',
//                s_createTimeEnd: ''
            }
        },
        methods: {
            //---
//            test:function () {
//                console.log(this.s_updateTimeEnd)
//              let time1 = this.timeExchange2(this.s_updateTimeEnd)
////              console.log(time1)
//            },
            //清空搜索选项
            clearOption:function () {
                this.s_driverName=''
                this.s_certificateType=''
                this.s_certificateNo=''
                this.s_driverStatus=''
                this.s_realName=''
                this.s_registerIp=''
                this.s_driverType=''
                this.s_createTime=''
                this.s_areaName=''
                this.s_licenseNumber=''
                this.s_licensePlates=''
                this.s_updateUserName=''
                this.s_userName=''
                this.s_updateTimeStart=''
                this.s_updateTimeEnd=''
//                this.s_createTimeStart=''
//                this.s_createTimeEnd=''
            },
            // 时间转换时间戳
            // 关闭高级搜索面板
            offSelf: function () {
                this.$emit('off_sm')
            },
            //查询参数
            searchOption: function () {
                let opts = {
                    "driverName": this.s_driverName,     // 司机昵称
                    "certificateType": this.s_certificateType,      //证件类型 1身份证
                    "certificateNo": this.s_certificateNo,   // 证件号
                    "driverStatus": this.s_driverStatus,     //认证状态  0未认证  1审核中  2 认证失败 3 已认证
                    "realName": this.s_realName,     //真实名称
                    "registerIp": this.s_registerIp,     //注册ip地址
                    "driverType": this.s_driverType,     //司机类型 1签约司机 2加盟司机
                    "createTime": this.s_createTime,     //创建时间
                    "areaName": this.s_areaName,     //地区名
                    "licenseNumber": this.s_licenseNumber,     //行驶证号
                    "licensePlates": this.s_licensePlates,     //车牌号
                    "updateRealName": this.s_updateUserName,     //修改人
                    "userName": this.s_userName,     //app用户名
                    "updateTimeStart": this.s_updateTimeStart,    //修改开始时间
                    "updateTimeEnd": this.s_updateTimeEnd,    //修改结束时间
//                    "createTimeStart": this.s_createTimeStart,    //创建开始时间
//                    "createTimeEnd": this.s_createTimeEnd
                }
                return opts
            },
            //高级查询
            search_More: function () {
                let opt = this.searchOption()
                // 审核状态，证件类型转换为int
                if(opt.certificateType!==''){
                    opt.certificateType = parseInt(opt.certificateType)
                }
                if(opt.driverStatus!==''){
                    opt.driverStatus = parseInt(opt.driverStatus)
                }
                if(opt.driverType !== ''){
                    opt.driverType = parseInt(opt.driverType)
                }
//                身份证号验证
                if (opt.certificateNo !== '') {
                    let reg1 = /(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/;
                    if(reg1.test(opt.certificateNo) === false) {
                        layer.msg("身份证输入不合法");
                        return false;
                    }
                }

                // IP地址验证
                if (opt.registerIp !== '') {
                    let reg_ip = /(2(5[0-5]{1}|[0-4]\d{1})|[0-1]?\d{1,2})(\.(2(5[0-5]{1}|[0-4]\d{1})|[0-1]?\d{1,2})){3}/g;
                    if(!reg_ip.test(opt.registerIp)) {
                        layer.msg('输入的IP地址无效');
                        return false
                    }
                }

                // 转换为时间戳 getTime()
                if(opt.createTime!==''){
                    opt.createTime = this.timeExchange(opt.createTime)
                }
                if(opt.updateTimeStart!==''){
                    opt.updateTimeStart = this.timeExchange(opt.updateTimeStart)
                }
                if(opt.updateTimeEnd!==''){
                    opt.createTime = this.timeExchange(opt.createTime)
                }
//                if(opt.createTimeStart!==''){
//                    opt.createTimeStart = this.timeExchange(opt.createTimeStart)
//                }
//                if(opt.createTimeEnd!==''){
//                    opt.createTimeEnd = this.timeExchange(opt.createTimeEnd)
//                }
//                console.log(opt)
                let opt1 = {}
                for (let k in opt){
                    if(opt[k] !== '') {
                        opt1[k] = opt[k]
                    }
                }
//                console.log(opt1)
                let data = {
                    opt:opt1,
                    cur:1
                }
                this.$emit('submitOption',data)
            }
        }
    }
</script>

<style scoped>
.search_input {
    display: block;
    width: 80%;
    height: 34px;
    padding: 6px 12px;
    font-size: 14px;
    line-height: 1.42857143;
    color: #555;
    background-color: #fff;
    background-image: none;
    border: 1px solid #ccc;
    border-radius: 4px;
}
.search_more_box{
    display: flex;
    border-bottom: 1px dotted;
    padding-bottom: 20px;
    margin-bottom: 20px;
    flex-wrap: wrap;
}
.searchBtn{
    margin:0 4px;
}
.btnGroup{
    display: flex;
    justify-content: flex-start;
    align-items: center;
    padding:12px 0;
}
</style>
